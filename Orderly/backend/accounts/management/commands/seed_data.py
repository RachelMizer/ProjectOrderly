...
def ensure_variant(product: Product, name: str, unit_price: Decimal) -> ProductVariant:
    """
    Create/update a ProductVariant for (product, name) with:
      - required sku
      - required unit_price
    Also repairs older bad rows (blank sku / null unit_price).
    """
    ...
    # Repair/normalize if already exists
    dirty = False
    if not obj.sku:
        obj.sku = sku
        dirty = True
    ...
    # Keep seed deterministic: update unit_price if different
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(1, 3):
            ...
            UserProfile.objects.update_or_create(user=u, defaults={"role": UserRole.BUSINESS})

        # 5 customers
        for i in range(1, 6):
            u = make_user(f"customer{i}", f"customer{i}@example.com")
            UserProfile.objects.update_or_create(user=u, defaults={"role": UserRole.CUSTOMER})
            CustomerProfile.objects.update_or_create(
                ...
                defaults={
                    ...
                    "state": "NC",
                    "zipcode": f"2750{i}",
                    "phone": f"919-555-01{str(i).zfill(2)}",
                },
            )
        ...
        # ------------------------------------------------------------
        supplier_rows = [
            {"name": "Fresh Farms Co", "email": "contact@freshfarms.example", "phone": "919-555-1200"},
            {"name": "Triangle Produce", "email": "sales@triangleproduce.example", "phone": "919-555-1300"},
            {"name": "Carolina Wholesale", "email": "orders@carolinawholesale.example", "phone": "919-555-1400"},
            {"name": "Good Grain Supply", "email": "hello@goodgrain.example", "phone": "919-555-1500"},
        ]
        ...
        for it in inventory_rows:
            obj, _ = InventoryItem.objects.get_or_create(name=it["name"], defaults=it)
            # if exists, keep seed deterministic by updating quantities
            changed = False
            for k, v in it.items():
                if getattr(obj, k) != v:
                    setattr(obj, k, v)
        ...
        # ------------------------------------------------------------
        # 5) Catalog: categories, products, variants, modifiers
        # ------------------------------------------------------------
        ...
        # Products need category (required). Supplier is optional.
        products_spec = [
        ...
        for pname, cname, supp in products_spec:
            ...
            # Update deterministic fields if already exists
            changed = False
            if p.supplier != supp:
                p.supplier = supp
                changed = True
        ...
        # Variants must have sku + unit_price
        variants_spec = [
        ...
        # Modifiers: your schema ties groups to a *variant*
        if latte_small is not None:
            mg, _ = ModifierGroup.objects.get_or_create(
                variant=latte_small,
                name="Milk Type",
                defaults={"required": False, "min_selections": 0, "max_selections": 1},
            )
