...
Product Card Uses Radio Variants Or Hides Selector For Single Variant
    ...
    ${card_count}=    Get Element Count    css=.product-card
    Should Be True    ${card_count} > 0
...
Category Filter Can Be Changed
    ...
    Click Element    xpath=(//div[contains(@class,'filter')]//label[contains(normalize-space(.),'Coffee')]//input[@type='checkbox'])[1]
    Sleep    1s
...
View And Customize Navigates To Product Page
    Go To    ${STOREFRONT_URL}
    Wait Until Page Contains Element    css=.product-card .view-link    10s
    Click Element    xpath=(//a[contains(@class,'view-link') and normalize-space()='View & Customize'])[1]
